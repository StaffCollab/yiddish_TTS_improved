...
if __name__ == "__main__":
    print("🏗️ Conservative Yiddish TTS Training")
    print("Full model architecture with system-safe training practices")
    
    try:
        tokenizer_file = train_conservative_yiddish()
        print(f"\n🎉 Conservative training session successful!")
        print(f"📁 Tokenizer: {tokenizer_file}")
        print(f"🧠 Full 2M parameter model preserved")
    except KeyboardInterrupt:
        print("\n⏸️  Training interrupted by user")
    except Exception as e:
        print(f"\n❌ Training failed: {e}")
        print("The full model requires more resources - try the safe version if needed") 